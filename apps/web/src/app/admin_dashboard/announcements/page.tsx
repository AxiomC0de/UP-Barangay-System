        ...
        <Box sx={{ px: 2, py: 1.5 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <Button
              variant="contained"
              startIcon={<AddIcon />}
              onClick={() => router.push('/admin_dashboard/announcements/new')}
              sx={{
                bgcolor: '#7B1113',
                color: '#fff',
                textTransform: 'none',
      ...
      <Box sx={{ px: 2, py: 1.5, display: 'flex', gap: 1, flexWrap: 'wrap', borderBottom: '1px solid #2f3336' }}>
        {categories.map((cat) => (
          <Chip
            ...
            sx={{
              bgcolor: categoryFilter === cat.value ? '#7B1113' : '#202327',
              color: categoryFilter === cat.value ? '#fff' : '#e7e9ea',
              '&:hover': {
                bgcolor: categoryFilter === cat.value ? '#9B1315' : '#2f3336',
              },
            }}
          />
        ))}
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1, flexWrap: 'wrap' }}>
                    {announcement.isPinned && (
                      <PushPinIcon sx={{ fontSize: 16, color: '#7B1113' }} />
                    )}
                    <Typography
                      sx={{
                        fontWeight: 700,
      ...
      >
        <MenuItem
          onClick={() => {
            handleMenuClose();
            if (selectedAnnouncement) {
              router.push(`/admin_dashboard/announcements/${selectedAnnouncement.id}/edit`);
            }
          }}
